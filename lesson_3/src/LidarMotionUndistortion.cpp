...
#include "LaserUndistortion/ChampionNavLaserScan.h"
...
static void calibratiorHandler(int sig)
{
    ros::shutdown();
}
...
public:
    ...
    void ScanCallBack(const champion_nav_msgs::ChampionNavLaserScanPtr &scan_msg)
    {
        ...
        tf::TransformListener *tmp_tf = tf_;
        if (!getLaserPose(visualPose, startTime, tmp_tf))
        {
                      ...
                      tf::TransformListener *tf)
    {
        ...
        try
        {
            if (!tf->waitForTransform("/odom", "/base_laser", dt, ros::Duration(0.5))) // 0.15s 的时间可以修改
            {
                ROS_ERROR("LidarMotion-Can not Wait Transform()");
                return false;
            }
            tf->transformPose("/odom", robot_pose, odom_pose);
        }
        ...
        int &beam_number)
    {
        //TODO
        //1 获取全局位姿
        tf::Quaternion quaternion_base = frame_base_pose.getRotation();
        tf::Quaternion quaternion_start = frame_start_pose.getRotation();
        tf::Quaternion quaternion_end = frame_end_pose.getRotation();

        tf::Vector3 pose_base = frame_base_pose.getOrigin();
        tf::Vector3 pose_start = frame_start_pose.getOrigin();
        tf::Vector3 pose_end = frame_end_pose.getOrigin();

        tf::Transform transform_base(quaternion_base, pose_base);
        tf::Transform transform_start(quaternion_start, pose_start);
        tf::Transform transform_end(quaternion_end, pose_end);

        //2 获取起止点位姿 相对于雷达坐标系
        tf::Transform laser_start = transform_base.inverse() * transform_start;
        tf::Transform laser_end = transform_base.inverse() * transform_end;

        quaternion_start = laser_start.getRotation();
        quaternion_end = laser_end.getRotation();

        pose_start = laser_start.getOrigin();
        pose_end = laser_end.getOrigin();
        std::cout << "pose_start: "
                  << "x: " << pose_start[0] << " y: " << pose_start[1] << "z: " << pose_start[2] << std::endl;
        std::cout << "pose_end: "
                  << "x: " << pose_end[0] << " y: " << pose_end[1] << "z: " << pose_end[2] << std::endl;

        for (int index = startIndex; index < startIndex + beam_number; index++)
        {
            //滤掉空点
            if (ranges[index] < 0.05 || std::isnan(ranges[index]) || std::isinf(ranges[index]))
                continue;

            //3 线性插值 获取 x y yaw 的差值
            double time_index = (index - startIndex) / (beam_number * 1.0);
#if DEBUG_LIDAR
            std::cout << "time index: " << time_index << std::endl;
#endif
            tf::Quaternion quaternion_interpolation = quaternion_start.slerp(quaternion_end, time_index);
            tf::Vector3 pose_interpolation = pose_start.lerp(pose_end, time_index);

            float yaw = tf::getYaw(quaternion_interpolation);

            //4-1 还原到二维笛卡尔坐标系
            double pose_x = ranges[index] * cos(angles[index]);
            double pose_y = ranges[index] * sin(angles[index]);

            //4-2 根据公式进行矫正
            tf::Matrix3x3 R_index(cos(yaw), -sin(yaw), 0,
                                  sin(yaw), cos(yaw), 0,
                                  0, 0, 1);

            tf::Vector3 pose_old(pose_x, pose_y, 1);
            tf::Vector3 pose_new = R_index * pose_old;
            pose_new = pose_new + pose_interpolation;

            ranges[index] = sqrt(pow(pose_new[0], 2) + pow(pose_new[1], 2));
            angles[index] = atan2(pose_new[1], pose_new[0]);
        }
        //end of TODO
    }
                           ...
                           tf::TransformListener *tf)
    {
        ...
        // 5ms来进行分段
        int interpolation_time_duration = 20 * 1000;
        ...
        if (!getLaserPose(frame_start_pose, ros::Time(start_time / 1000000.0), tf))
        {
            ROS_WARN("Not Start Pose,Can not Calib");
            return;
        }

        if (!getLaserPose(frame_end_pose, ros::Time(end_time / 1000000.0), tf))
        {
        ...
        for (int i = 0; i < beamNumber; i++)
        {
            ...
            if (mid_time - start_time > interpolation_time_duration || (i == beamNumber - 1))
            {
                ...
                //得到起点和终点的位姿
                //终点的位姿
                if (!getLaserPose(frame_mid_pose, ros::Time(mid_time / 1000000.0), tf))
                {
                ...
                {
                    //包含最后一个点
                    interp_count = i - start_index + 1;
                }
                else
                {
                    //防止中间的起止点被矫正两次
                    interp_count = i - start_index;
                }
...
{
    signal(SIGINT, calibratiorHandler);
    ros::init(argc, argv, "LidarMotionCalib", ros::init_options::NoSigintHandler);
