...
void Scan2::scanCallBack(const sensor_msgs::LaserScan::ConstPtr &_laserScanMsg)
{
    ...
    // std::cout<<"d_point_odom: "<< d_point_odom[0] << " " << d_point_odom[1] << " " << d_point_odom[2] <<std::endl;
...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    //TODO:
    now_pos = odom_pose;
    Eigen::MatrixXd transform_last(3,3);  //临时的变量
    Eigen::MatrixXd transform_now(3,3);  //临时的变量
    Eigen::MatrixXd transform_matrix(3,3);  //临时的变量

    double c,s;
    c = cos(last_pos(2));
    s = sin(last_pos(2));
    transform_last<<c, -s, last_pos(0),
                      s, c,  last_pos(1),
                     0,  0, 1;

    c = cos(now_pos(2));
    s = sin(now_pos(2));
    transform_now<<c, -s, now_pos(0),
                   s, c, now_pos(1),
                   0,  0, 1;
    
    

    transform_matrix =  transform_last.inverse() * transform_now;

    d_pos[0] = transform_matrix(0,2);
    d_pos[1] = transform_matrix(1,2);
    d_pos[2] = atan2(transform_matrix(1,0), transform_matrix(0,0));
    //end of TODO:
